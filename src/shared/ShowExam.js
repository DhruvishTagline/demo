...
//       <div>
//         {createExamFields.map((field, i) => (
//           <InputField fieldData={field} subjectName={subjectName} key={i} />
//         ))}
//       </div>
//       {error?.answer !== undefined && <span className='text-red-500 text-sm'>{error.answer}</span>}
//       {error?.sameOption !== undefined && <span className='text-red-500 text-sm'>{error.sameOption}</span>}
...
//         <button
//           onClick={handleNextQuestion}
//           disabled={currQuestion === totalQue}
//           className={`bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 ml-2 rounded focus:outline-none focus:shadow-outline ${currQuestion === 14 || currQuestion === 7 ? 'opacity-50 cursor-not-allowed' : ''}`}
//         >
//           Next
//         </button>
//       </div>
